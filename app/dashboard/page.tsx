...
export default function DashboardPage() {
  ...
  for (const e of entries) {
    ...
    if (!seen.has(key)) {
      ...
      groups.push({
        dateKey: key,
        label: dayLabel(key),
        entries: entries.filter((x) => new Date(x.started_at).toDateString() === key),
      });
    }
  }
  ...
  const resume = (entry: EntryRow) => {
    ...
    window.dispatchEvent(new CustomEvent("ouratime:resume", {
      detail: {
        description: entry.description,
        project: entry.project_id && entry.projects
          ? { id: entry.project_id, ...entry.projects }
          : null,
        tags: entry.time_entry_tags.map((t) => t.tags).filter((t): t is Tag => t !== null),
    ...
    <main className={styles.page}>
      ...
      <div className={styles.statsRow}>
        <div className={styles.card}>
          ...
          <p className={`${styles.statValue} ${styles.statValueTeal}`}>{fmtHHMM(todaySecs)}</p>
        </div>
        ...
        <div className={styles.card}>
          ...
          <p className={`${styles.statValue} ${styles.statValueTeal}`}>{fmtHHMM(billableSecs)}</p>
        </div>

        <div className={styles.card}>
          <p className={styles.statLabel}>Active Projects</p>
          <p className={styles.statValue}>{String(activeProjects).padStart(2, "0")}</p>
        </div>
      </div>
            ...
            return (
              <div key={g.dateKey} className={styles.group}>
                <div className={styles.groupHeader}>
                  ...
                  <span className={styles.groupTotal}>{fmtHHMMSS(groupTotal)}</span>
                </div>
                ...
                <div className={styles.entryCard}>
                  {g.entries.map((entry, idx) => {
                    ...
                    return (
                      ...
                      >
                        {/* Icon */}
                        <div className={`${styles.entryIcon} ${running ? styles.entryIconRunning : ""}`}>
                          {running ? (
                          ...
                          <div className={styles.entryMeta}>
                            {entry.projects && (
                              <span className={styles.projectChip}>
                                <span className={styles.projectDot} style={{ background: entry.projects.color }} />
                                {entry.projects.name}
                              </span>
                            )}
                            {tags.map((tag) => (
                        ...
                        <div className={styles.entryRight}>
                          ...
                          {running && <div className={styles.playBtnSpacer} />}
                        </div>
                      </div>
                    );
                  })}
