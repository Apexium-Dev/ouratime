...
interface ProjectSlice {
  id: string; name: string; color: string; secs: number; pct: number;
}
...
function getPeriodRange(period: Period): { from: Date; to: Date } {
  ...
  switch (period) {
    case "week": {
      ...
      return { from: new Date(d.getFullYear(), d.getMonth(), d.getDate()), to: eod };
    }
    case "lastweek": {
      const d = new Date(eod);
    ...
    case "3m": {
      const from = new Date(eod); from.setMonth(from.getMonth() - 3); from.setHours(0,0,0,0);
      return { from, to: eod };
    }
    default: {
      const from = new Date(eod); from.setDate(from.getDate() - 29); from.setHours(0,0,0,0);
      return { from, to: eod };
    }
  }
}
...
function arc(cx: number, cy: number, oR: number, iR: number, a1: number, a2: number) {
  const os = polar(cx, cy, oR, a1), oe = polar(cx, cy, oR, a2);
  const is = polar(cx, cy, iR, a1), ie = polar(cx, cy, iR, a2);
  const lg = a2 - a1 > 180 ? 1 : 0;
  return `M${os.x} ${os.y} A${oR} ${oR} 0 ${lg} 1 ${oe.x} ${oe.y} L${ie.x} ${ie.y} A${iR} ${iR} 0 ${lg} 0 ${is.x} ${is.y}Z`;
}

function DonutChart({ slices, totalSecs, hoveredId, onHover }: {
  slices: ProjectSlice[]; totalSecs: number;
  hoveredId: string | null; onHover: (id: string | null) => void;
}) {
  const cx = 110, cy = 110, oR = 88, iR = 60, GAP = 1.8;
  let angle = 0;
  ...
  return (
    <svg viewBox="0 0 220 220" className={styles.donutSvg}>
      ...
      {hov && (
        <text x={cx} y={cy + 28} textAnchor="middle" className={styles.donutPct}>
          {hov.pct.toFixed(1)}%
        </text>
      )}
    </svg>
...
// ── Bar chart ────────────────────────────────────────────────────────────────
function BarChart({ data, period }: { data: { label: string; secs: number }[]; period: Period }) {
  const [hov, setHov] = useState<number | null>(null);
  const W = 560, H = 210, PL = 36, PB = 32, PT = 14, PR = 8;
  ...
  const yTicks = maxH <= 4
    ...
    : maxH <= 8
      ...
      : [0, Math.round(maxH / 4), Math.round(maxH / 2), Math.round(maxH * 3 / 4), maxH];

  const step = cW / data.length;
  const bW = Math.max(3, step * 0.55);
  const isWeek = period === "week" || period === "lastweek";
  const every = data.length <= 7 ? 1 : data.length <= 14 ? 2 : data.length <= 31 ? 7 : 14;
        ...
        return (
          <g key={h}>
            <line x1={PL} y1={y} x2={W - PR} y2={y}
              stroke={h === 0 ? "#e0ddd9" : "#f3f1ef"} strokeWidth={h === 0 ? 1.5 : 1} />
            <text x={PL - 5} y={y + 4} textAnchor="end" className={styles.axisLbl}>{h}h</text>
          </g>
        );
      })}
      ...
      {data.map((d, i) => {
        ...
        return (
          <g key={i} onMouseEnter={() => setHov(i)} onMouseLeave={() => setHov(null)}>
            <rect x={PL + i * step} y={PT} width={step} height={cH + PB} fill="transparent" />
            {bH > 0 && (
              <rect x={x} y={y} width={bW} height={bH} rx={Math.min(4, bW / 2)}
                fill="#008080"
            ...
            {isHov && d.secs > 0 && (() => {
              ...
              return (
                <g>
                  <rect x={tx - 28} y={ty} width={56} height={20} rx={5} fill="#050505" />
                  <text x={tx} y={ty + 14} textAnchor="middle" className={styles.tooltip}>
                    {fmtH(d.secs)}
                  </text>
                </g>
              );
            ...
            {i % every === 0 && (
              <text x={PL + i * step + step / 2} y={H - 6} textAnchor="middle" className={styles.axisLbl}>
...
export default function ReportsPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data } = await supabase
        .from("time_entries")
        .select("id, started_at, stopped_at, duration, billable, project_id, projects(name,color)")
  ...
  const { totalSecs, billableSecs, slices, bars } = useMemo(() => {
    let totalSecs = 0, billableSecs = 0;
    const pMap: Record<string, { name: string; color: string; secs: number }> = {};

    for (const e of entries) {
      const s = e.duration ?? 0;
      totalSecs += s; if (e.billable) billableSecs += s;
      const key = e.project_id ?? "__none__";
      if (!pMap[key]) pMap[key] = { name: e.projects?.name ?? "No project", color: e.projects?.color ?? NO_PROJECT_COLOR, secs: 0 };
      pMap[key].secs += s;
    }

    const slices: ProjectSlice[] = Object.entries(pMap)
      .map(([id, v]) => ({ id, ...v, pct: totalSecs > 0 ? (v.secs / totalSecs) * 100 : 0 }))
    ...
    while (cur <= to) {
      bars.push({
        label: isWeek ? DAYS[cur.getDay()] : `${MONS[cur.getMonth()]} ${cur.getDate()}`,
        ...
        <>
          ...
          <div className={styles.statsRow}>
            ...
            <div className={styles.statCard}>
              ...
              <p className={styles.statVal}>{String(entries.length).padStart(2,"0")}</p>
            </div>
          </div>
          ...
          <div className={styles.breakdownCard}>
            ...
            <div className={styles.breakList}>
              {slices.map((s, i) => (
                <div key={s.id} className={styles.breakRow} style={{ animationDelay: `${i * 40}ms` }}>
                  <div className={styles.breakInfo}>
                    <span className={styles.breakDot} style={{ background: s.color }} />
                    <span className={styles.breakName}>{s.name}</span>
                  </div>
                  <div className={styles.breakTrack}>
                    <div className={styles.breakFill} style={{ width: `${s.pct}%`, background: s.color }} />
                  </div>
