...
export default function CalendarPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data } = await supabase
        .from("time_entries")
        .select("id, description, started_at, stopped_at, duration, billable, projects (name, color)")
  ...
  const selLabel = isSameDay(selectedDay, today) ? "Today" :
              ...
              >
                ...
                {colors.length > 0 && (
                  <div className={styles.cellDots}>
                    {colors.map((c, idx) => (
                      <span key={idx} className={styles.cellDot} style={{ background: c }} />
                    ))}
                  </div>
                )}
              </button>
      ...
      <div className={styles.dayPanel}>
        ...
        {loading ? (
          ...
          <div className={styles.panelEmptyState}>
            ...
            <p>No entries tracked{isSameDay(selectedDay, today) ? " today" : " this day"}.</p>
          </div>
        ) : (
            ...
            {selEntries.map((entry) => (
              <div key={entry.id} className={styles.entryRow}>
                ...
                <div className={styles.entryRight}>
                  <span className={`${styles.entryDur} ${entry.billable ? styles.entryDurBillable : ""}`}>
                    {entry.stopped_at
