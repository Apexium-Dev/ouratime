...
export default function InvoicesPage() {
  ...
  useEffect(() => {
    async function load() {
      const { data } = await supabase
        .from("invoices")
        .select("id, number, client_name, client_email, status, issue_date, due_date, currency, tax_rate, invoice_items(quantity, rate)")
  ...
  const totalInvoiced = invoices.filter(i => i.status !== "draft").reduce((s, i) => s + invoiceTotal(i), 0);
  ...
  return (
    <main className={styles.page}>
      ...
      <div className={styles.topBar}>
        ...
        <button className={styles.newBtn} onClick={handleNew} disabled={creating}>
      ...
      <div className={styles.statsRow}>
        <div className={styles.statCard}>
          ...
          <p className={`${styles.statVal} ${styles.teal}`}>{fmt(totalInvoiced)}</p>
        </div>
        <div className={styles.statCard}>
        ...
        <div className={styles.statCard}>
          ...
          <p className={`${styles.statVal} ${outstanding > 0 ? styles.orange : ""}`}>{fmt(outstanding)}</p>
        </div>
        <div className={styles.statCard}>
          <p className={styles.statLbl}>Total invoices</p>
          <p className={styles.statVal}>{String(invoices.length).padStart(2, "0")}</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className={styles.emptyState}>
          ...
          {filter === "all" && <p>Click "New invoice" to create your first one.</p>}
        </div>
      ) : (
          ...
          {filtered.map(inv => (
            ...
            >
              ...
              <div className={styles.clientCell}>
                ...
                {inv.client_email && <span className={styles.clientEmail}>{inv.client_email}</span>}
              </div>
              <span className={styles.dateCell}>{fmtDate(inv.issue_date)}</span>
              <span className={`${styles.dateCell} ${inv.due_date && new Date(inv.due_date) < new Date() && inv.status === "sent" ? styles.overdue : ""}`}>
                {fmtDate(inv.due_date)}
              </span>
              <span className={styles.amountCell}>{fmt(invoiceTotal(inv), inv.currency)}</span>
