...
export function EditEntryModal({ entry, onClose, onSave, onDelete }: Props) {
  ...
  const handleSave = async () => {
    ...
    const { data: updated, error } = await supabase
      ...
      .eq("id", entry.id)
      .select(
        "id, description, started_at, stopped_at, duration, billable, project_id, projects(name, color)",
      )
      .single();
    ...
    if (!error && updated) {
      ...
      const projectsValue = updated.projects
